...
def get_custom_word_ids(input_ids, offset_mapping, tsv_spans, special_ids):
    ...
    for token_id, (char_start, char_end) in zip(input_ids, offset_mapping):
        ...
        for i, (s, e) in enumerate(tsv_spans):
            # Overlap check, not strict containment: byte-level BPE
            # tokenizers (GPT-2/Llama/Gemma style) fold the leading space
            # before a word into that word's first subtoken, e.g. "ĠJune"
            # gets offsets starting at the space character rather than at
            # "J". A strict s <= char_start <= char_end <= e check would
            # then miss the match entirely.
